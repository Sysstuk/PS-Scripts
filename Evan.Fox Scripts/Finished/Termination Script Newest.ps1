...
#Runs the AD/AAD sync on UE1CIF-P003W. If it fails, it will try it again
function dcsync {
    try{
        Write-Host "Attempting UE1CIF-P003W sync. Please wait 90 seconds for sync"
        $adcreds = get-credential
        Invoke-Command -ComputerName 'UE1CIF-P003W' -ScriptBlock {Start-AdSyncSyncCycle -Policytype Delta} -Credential $adCreds
...
function aadterm {
    ...
    $empname = "suzanne.csendom@springeq.com"
    ...
    #Checks if mailbox has used more than 48GB of space and assigns Exchange Online Plan 2 license if so
    $mbSize = ((get-mailboxstatistics -identity $upn).TotalItemSize.Value).tostring()
    if($mbSize -like "*GB*"){
        $mbSize = $mbSize.Split(' ')
        if($mbSize[0] -gt 48){
            $EOSku = Get-MgSubscribedSku -All | Where SkuPartNumber -eq 'EXCHANGEENTERPRISE'
            Set-MgUserLicense -UserId "$upn" -AddLicenses @{SkuId = $EOSku.SkuId} -RemoveLicenses @()
        }
    }
