...
function aadterm {
    ...
    foreach($item in $roleGroups) {
        if ($item -ne "7b181853-5483-44f0-ab96-fc73261a73cc") {
            if (((Get-AzureADMSGroup -id $item).GroupType -ne "Unified") -and ((Get-AzureADMSGroup -id $item).MailEnabled -eq $true)) {
                Remove-DistributionGroupMember -Identity $item -Member $upn
            }
            elseif ((Get-AzureADMSGroup -id $item).GroupType -contains "Unified") {
